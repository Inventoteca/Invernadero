...
void drawUI5() {
  ...
  tft.drawString("Color ", 40, 10);
  tft.drawString("R:" + String(val_r, 0) + " ", 110, 10);
  tft.drawString("G:" + String(val_g, 0) + " ", 170, 10);
  tft.drawString("B:" + String(val_b, 0) + " ", 230, 10);
...
void loopUI5() {
  if (pantalla_inicia) {
    ...
    for (uint8_t i = 0; i < 4; i++) { //revisar los botones
      ...
      // Mover sliders y cambiar salida
      if (b5[i].isPressed()) {
        switch (i)
        {
          case 0: //R
          ...
          case 2: //B
            ...
            //tft.drawString(val, 230, 10);
            //Serial.println(val);

            //salida PWM
            analogWrite(PIN_G, round(val_b * 25.5));
            break;
        }
      }
